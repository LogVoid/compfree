...
def add_product(name: str, description: str, img: str = None) -> None:
    new_product = Product(name=name, description=description, img=img)
    db.session.add(new_product)
    db.session.commit()
    print(f'Product "{name}" added to the database!')

def edit_product(pid: int, name: str = None, description: str = None, img: str = None) -> None:
...
def list_products() -> None:
    ...
    if prods:
        for prod in prods:
            print(f"ID: {prod.id} Name: {prod.name} Description: {prod.description} Image: {prod.img}")
    else:
...
def main() -> None:
    ...
    with app.app_context():
        if args.command == "add":
            add_product(args.name, args.description, args.img)
        elif args.command == "edit":
            edit_product(args.id, args.name, args.description, args.img)
